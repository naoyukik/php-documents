/**
 * (PECL mysqlnd-uh >= 1.0.0-alpha)<br/>
 * @link http://php.net/manual/en/mysqlnduhconnection.useresult.php
 * @param mysqlnd_connection $connection [optional] <p> TODO DESCRIPTION </p>
 * @return resource Resource of type Mysqlnd Resultset (internal only - you must not modify it!).
 * The documentation may also refer to such resources using the alias name
 * mysqlnd_resultset.Mysqlnd Resultset (internal only - you must not modify it!)mysqlnd_resultset
 */
function MysqlndUhConnection.useResult($connection) {}